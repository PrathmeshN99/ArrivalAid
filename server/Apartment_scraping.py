...
for url in urls:
    ...
    for element in k:
        ...
        # Print the extracted information for this element
    #     print(f"Property Title: {title_text}")
    #     print(f"Location: {location_text}")
    #     print(f"Rent Price: {rent_price_text}")
    #     print(f"Deposit Price: {deposit_price_text}")
    #     print(f"Built-up Area: {builtup_area_text}")
    ...
    for page in range(20):
        bar.update(page+1)
        page += 1
        link = "https://www.nobroker.in/property/rent/pune/Bibwewadi?searchParam=W3sibGF0IjoxOC40NjkwMjEzLCJsb24iOjczLjg2NDA5NDQsInBsYWNlSWQiOiJDaElKalhhRlFKZnF3anNSZHVLa2IzMF9mUlkiLCJwbGFjZU5hbWUiOiJCaWJ3ZXdhZGkifV0=&radius=2.0&sharedAccomodation=0&city=pune&locality=Bibwewadi"+str(page)
...
# Calculate recommendation scores based on cluster similarity
# apartment_df['Score'] = kmeans.transform(preference_vector.reshape(1, -1)).min()

# # Sort apartments by score in descending order
# ranked_apartments = apartment_df.sort_values(by='Score', ascending=False)

# # Display the ranked apartments
# ranked_apartments[['Apartment_Name','Score']]
# apartment_df[['Apartment_Name','Cluster']]
